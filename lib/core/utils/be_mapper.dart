...
class BeMapper {
  ...
  /// is checked against known codes and mapped to an existing domain
  /// exception when one exists; everything else in that bucket falls back to
  /// the matching generic [AppErrorCode].
  static AppException mapBeError(BeErrorData errorData) {
    final keys = errorData.errorMessageMap.keys;
    final code = keys.isEmpty ? null : keys.first;

    return switch (errorData.statusCode) {
      // Global bucket — session/token invalid regardless of which code
      // fired: envNotAvailable, jwtExpired, jwtValidationFailed,
      // notAuthorized, noUserAuth. See AuthInterceptor.onResponse for the
      // actual side-effect (logout) — this mapper only produces the
      // exception.
      ...
      // Case-by-case — user IS authenticated, just not allowed for this
      // action: notAuthorized (role/path denial), userNotActive,
      // notRecipeOwner, cannotChangeRoleToAdmin,
      // cannotChangeOwnActiveStatus.
      // No dedicated domain exception yet for any of these — add one here
      // as each case needs a distinct message.
      ...
      400 => switch (code) {
        ...
        // most of these are already caught client-side by AppValidation
  ...
  /// Unwraps a parsed [BeGeneralResponse]: throws the mapped [AppException]
  /// when `errorData` is populated, otherwise returns `data`.
  static T mapBeResponse<T>(BeGeneralResponse<T, dynamic> response) {
    if (response.errorData != null) {
      throw mapBeError(response.errorData!);
    }
