...
class _EasySectionState extends ConsumerState<EasySection> {
  ...
  Future<void> _loadMore() async {
    final hasMore = ref.read(easyMealsProvider).value?.hasMore ?? false;
    if (_isLoadingMore || !hasMore) return;
    setState(() => _isLoadingMore = true);
    try {
      await ref.read(easyMealsProvider.notifier).loadMore();
    ...
    return switch (meals) {
      ...
      AsyncData(:final value) => Padding(
        ...
        child: GcSectionView(
          ...
          subtitle: AppLocale.getLabels(context).homeSectionEasySubtitle,
